<header>
  ...
  <div class="search-bar row bg-light p-2 my-2 rounded-4">
    <div class="col-11 col-md-11">
      <form id="search-form" class="text-center d-flex">
        <input type="text" name="search" class="form-control border-0 bg-transparent"
          placeholder="Search for more than 20,000 products">
        <button style="margin-right:0px; width: 30px" type="submit" class="btn btn-link p-0 ms-2">
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', async function (e) {
      ...
      try {
        const response = await fetch(`http://localhost/webbangiay/api/shoe/search?q=${encodeURIComponent(query)}`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok) {
          ...
          alert(result.message || 'Không tìm thấy sản phẩm');
        }
